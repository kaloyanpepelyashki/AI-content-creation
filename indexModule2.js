...
const musicExtractor = require("./MusicKeywordsExtractor.js");
...
const writeToFile = (content, purpose) => {
  const fileName = `Exports/solderingVideo/${purpose}.txt`;
  try {
...
async function main() {
  ...
  try {
    ...
    try {
      console.log(
        "Options passed to the script generator: ",
        sanitizedOutputFirst
      );
      const requestSecond = await openAi.chat.completions
        ...
        .then((response) => {
          if (
            ...
            console.log(
              "Generated content before extraction: ",
              generatedContent
            );
            const script = scriptExtractor(generatedContent);
            const musicKeyWords = musicExtractor(
              response.choices[0].message.content
            );

            console.log("Music keyword: ", musicKeyWords);
